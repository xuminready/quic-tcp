...
impl Session {
        ...
        match self.conn.stream_send(stream_id, data_to_write, fin) {
            Ok(written) => {
                ...
                if pending.written == pending.data.len() {
                    ...
                    debug!("Partially flushed {} bytes to QUIC stream {}", written, stream_id);
                    FlushStatus::Pending
                }
            }
            ...
            Err(e) => {
                error!("Failed to flush pending QUIC write for stream {}: {:?}", stream_id, e);
                self.quic_partial_writes.remove(&stream_id);
        ...
        match tcp_stream.write(data_to_write) {
            Ok(written) => {
                ...
                if pending.written == pending.data.len() {
                    ...
                    debug!("Partially flushed {} bytes to TCP stream {}", written, stream_id);
            ...
            Err(e) => {
                error!("Failed to flush pending TCP write for stream {}: {:?}", stream_id, e);
                self.tcp_partial_writes.remove(&stream_id);
                Err(e)
            }
        }
    ...
    pub fn forward_tcp_to_quic(&mut self, stream_id: u64, poll: &mut mio::Poll) -> io::Result<bool> {
        ...
        let Some(tcp_stream) = self.tcp_streams.get_mut(&stream_id) else {
            return Err(io::Error::new(io::ErrorKind::NotFound, "TCP stream not found"));
        };
                    ...
                    if self.conn.is_established() {
                        match self.conn.stream_send(stream_id, &buf[..n], false) {
                            Ok(written) => {
                                ...
                                if written < n {
                                    debug!("QUIC stream {} accepted only {}/{} bytes, buffering remainder", stream_id, written, n);
    ...
    pub fn forward_quic_to_tcp(&mut self, stream_id: u64, poll: &mut mio::Poll) -> io::Result<bool> {
        debug!("Forwarding QUIC -> TCP for stream {}", stream_id);

        if !self.opened_streams.contains(&stream_id) {
            debug!("Stream {} not opened yet, skipping forward_quic_to_tcp", stream_id);
            return Ok(false);
        }
        ...
        'recv: loop {
            match self.conn.stream_recv(stream_id, &mut buf) {
                Ok((n, fin)) => {
                    debug!("Read {} bytes from QUIC stream {} (fin={})", n, stream_id, fin);
                    if n > 0 {
                        match tcp_stream.write(&buf[..n]) {
                            Ok(written) => {
                                if written < n {
                                    debug!("TCP stream {} accepted only {}/{} bytes, buffering remainder", stream_id, written, n);
...
pub fn flush_quic_to_udp(conn: &mut quiche::Connection, udp_socket: &UdpSocket) -> io::Result<bool> {
...
pub fn get_quic_config() -> quiche::Config {
    ...
    config.set_application_protos(&[b"hq-interop", b"hq-29", b"hq-28", b"hq-27", b"http/0.9"]).unwrap();
